...
describe("Qubit Library Test Suite", () => {
  ...
  describe("Optimizer", () => {
    ...
    test("10. Merges RZ rotations", () => {
      const ops = [
        { gate: "RZ", qubit: q1, params: [0.1] },
        { gate: "RZ", qubit: q1, params: [0.2] },
      ];
    ...
    test("11. Merges RX rotations", () => {
      const ops = [
        { gate: "RX", qubit: q1, params: [Math.PI] },
        { gate: "RX", qubit: q1, params: [Math.PI] },
      ];
      expect(Optimizer.prune(ops)).toHaveLength(0);
    });
    ...
    test("16. Prunes U3(0,0,0) identity", () => {
      expect(
        Optimizer.prune([{ gate: "U3", qubit: q1, params: [0, 0, 0] }]),
      ).toHaveLength(0);
    });

    test("17. Commutation: RZ through CZ", () => {
      const ops = [
        { gate: "RZ", qubit: q1, params: [0.5] },
        { gate: "CZ", qubit: [q1, q2] },
        { gate: "RZ", qubit: q1, params: [0.5] },
      ];
    ...
    test("20. Prunes 2*PI rotations", () => {
      expect(
        Optimizer.prune([{ gate: "RY", qubit: q1, params: [2 * Math.PI] }]),
      ).toHaveLength(0);
    });
  });
  ...
  describe("Simulator Core Operations", () => {
    ...
    test("36. RZZ: functional check", () => {
      Q.use(2, (q1, q2, ops) => {
        ops.h(q1);
        ops.h(q2);
        ops.rzz(q1, q2, Math.PI);
        expect(() => ops.m(q1)).not.toThrow();
        ops.reset(q1);
        ops.reset(q2);
      });
    });
  ...
  describe("Algorithms", () => {
    ...
    test("54. VQE Ansatz execution", () => {
      Q.use(2, (q1, q2, ops) => {
        expect(() =>
          vqeAnsatz(ops, [q1, q2], [0, 0, 0, 0, 0, 0, 0, 0]),
        ).not.toThrow();
        ops.reset(q1);
        ops.reset(q2);
      });
    ...
    test("55. QAOA Layer execution", () => {
      Q.use(2, (q1, q2, ops) => {
        expect(() =>
          qaoaLayer(ops, [q1, q2], [[q1, q2]], 0.1, 0.2),
        ).not.toThrow();
        ops.reset(q1);
        ops.reset(q2);
      });
